...
describe('OptionBulkPaste', () => {
  it('splits pasted lettered options into entries', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Paste options from clipboard' }));
    fireEvent.change(screen.getByLabelText('Paste options'), {
      target: { value: 'A. Router\nB. Switch\nC. Hub\nD. Access Point' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Split into options' }));
  ...
  it('shows a clear error when the pasted text is not option-like', () => {
    ...
    expect(screen.getByText(/No option pattern detected/)).toBeInTheDocument();
  });
  ...
  it('remains hidden until opened', () => {
    ...
    expect(screen.queryByLabelText('Paste options')).not.toBeInTheDocument();
  });
});